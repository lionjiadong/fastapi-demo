...
from src.workflow.scheduler2.session import SessionManager, session_cleanup
from src.workflow.scheduler2.time_utils import NEVER_CHECK_TIMEOUT
    ...
    def __init__(self, model, Session, app=None, **kw):
        ...
        if expires:
            if isinstance(expires, dt.datetime):
                self.options["expires"] = maybe_make_aware(expires).astimezone(
                    self.app.timezone
                )
            else:
        ...
        self.last_run_at = maybe_make_aware(self.last_run_at).astimezone(
            self.app.timezone
        )
    ...
    def save(self, fields=tuple()):
        ...
        with session_cleanup(session):
            ...
            if obj:  # make sure object was not deleted
                ...
                logger.warning(
                    "couldn't update model %s, assuming it was deleted.",
                    self.model.name,
                )
...
class DatabaseScheduler(Scheduler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.schema = (
            kwargs.get("schema")
            or self.app.conf.get("beat_schema")
            or DEFAULT_BEAT_SCHEMA
        )
        self.engine_options = (
            kwargs.get("engine_options")
            or self.app.conf.get("beat_engine_options")
            or DEFAULT_BEAT_ENGINE_OPTIONS
        )
